...
def logg_inn():
  ...
  if request.method == "POST":
    ...
    content = requests.get('http://192.168.10.26:80/logg_inn', json={"navn": navn}).json()
...
def home_page(bruker_id):
  data = requests.get('http://192.168.10.26:80/get_todos', json={"bruker_id": bruker_id}).json()
...
def todo_post(bruker_id):
  ...
  requests.post('http://192.168.10.26:80/legg_til_todo', json={"bruker_id": bruker_id, "task": task})
...
def slett_todo(bruker_id, todo_id):
  requests.delete('http://192.168.10.26:80/slett_todo', json={"id": todo_id})
  return redirect(url_for('home_page', bruker_id=bruker_id))

@app.route('/rediger_todo/<bruker_id>/<todo_id>', methods = ["GET", "POST"])
def rediger_todo(bruker_id, todo_id):
  if request.method == "GET":
    data = requests.get('http://192.168.10.26:80/rediger_todo', json={"id": todo_id}).json()
    return render_template('rediger.html', bruker_id=bruker_id, todo_id=data["id"], todo=data["todo"])
  
  if request.method == "POST":
    task = request.form.get("task")
    requests.put('http://192.168.10.26:80/rediger_todo', json={"id": todo_id, "task": task})
    return redirect(url_for('home_page', bruker_id=bruker_id))
